...
const Samfund = () => {
    ...
    useEffect(() => {
        const fetchBlogPosts = async () => {
            try {
                // Filter entry IDs for "Samfund" based on routes containing "/samfund/sublink"
                const samfundEntryIds = Object.entries(entryIdMapping)
                    .filter(([key, value]) => value.includes("/samfund"))
                    .map(([key]) => key);

                const response = await client.getEntries({
                    content_type: "ingnNyheder",
                    'sys.id[in]': samfundEntryIds.join(',')
                });
